...
import { DELETE_DEVICES, DETAIL_OUTLET, GET_OUTLET, LIST_LOCATION_TRANSACTION, UPDATE_DEVICES } from 'action/actionTypes';
import { listLocation } from 'action/location';
import { resetAuthorize } from 'action/user';
import { listLocationTransaction } from 'action/transaction';

const loadingSelector = createLoadingSelector([GET_OUTLET,DETAIL_OUTLET,DELETE_DEVICES,UPDATE_DEVICES,LIST_LOCATION_TRANSACTION])
...
const mapDispatchToProps = (dispatch) => ({
    ...
    listLocationTransaction:(token) => dispatch(listLocationTransaction(token))
})
